...
public class ProfileController {
    ...
    @GetMapping
    public String displayProfile(@ModelAttribute("userProfileDTO") UserProfileDTO userProfileDTO, @ModelAttribute("user") User userLogged, Model model) {
    ...
    @PostMapping("/update")
    public String updateProfile(@Validated(ProfileUpdateValidation.class) UserProfileDTO userProfileDTO, BindingResult bindingResult, Model model) {
        model.addAttribute("user", userService.getLoggedUserbyEmail());
        if (bindingResult.hasFieldErrors()) {
            System.out.println(bindingResult.getFieldErrors());
            model.addAttribute("userProfileDTO", userProfileDTO);
            model.addAttribute("user", userService.getLoggedUserbyEmail());
            logger.error("Controller: Error. Form fields contain errors.  HTTP POST request /contacts/update URL");
            return "profile";
        }
        try {
            ...
            logger.warn("Controller: Warning. Passwords not match. HTTP POST request /profile/update URL");
            return "profile";
        }
        model.addAttribute("userProfileDTO", userProfileDTO);
